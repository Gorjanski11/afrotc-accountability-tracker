...
import type { PmtEvent, RosterPerson, TrainingObjectiveRef } from "../domain/types";

const COLLECTION = "completions";
const AUTO_EVALUATOR = "System (Auto: Absent)";

/** Required-proficiency cells are usually a single code ("P2"), occasionally a composite ("P1/P2") -- mirrors the TO's site's QuickLogScreen helper. */
...
/** The nearest code below the required one -- "almost made it" rather than always bottoming out at Ka. Mirrors the TO's site's QuickLogScreen helper. */
 ...
 * TO's-site-shared `completions` collection. An absence always overwrites whatever grade (if any)
 * was already logged for that cadet/objective/occurrence. Once set this way, nothing here ever
 * auto-reverts it -- not a later excuse (AE/PE), not a status correction away from "A" -- a human
 * has to re-grade it manually from Quick Log or Cadet Detail, same as any other completion.
 */
export function useAutoFailCompletions() {
  const applyAbsenceNotPass = useCallback(async (cadet: RosterPerson, pmtEvent: PmtEvent, catalogById: Map<string, TrainingObjectiveRef>) => {
