...
function mapEvent(id: string, data: Record<string, unknown>): PmtEvent {
  return {
    id,
...
/** Shared with the TO's site -- same `pmtEvents` collection, same Firebase project. This site never touches objectiveIds. */
export function usePmtEvents() {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      setEvents(snap.docs.map((d) => mapEvent(d.id, d.data())));
      setError(undefined);
    } catch (e) {
  ...
  const createEvent = useCallback(
    async (input: PmtEventInput) => {
      ...
      await refetch();
      return { id: ref.id, ...input, objectiveIds: [] } satisfies PmtEvent;
    },
    [refetch]
  );
